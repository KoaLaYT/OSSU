  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1)
               ...
               (error "MUPL ifgreater compare with non-number")))]
        [(mlet? e)
        ...
        [(call? e)
         (let ([v (eval-under-env (call-actual e) env)]
           ...
           (if (closure? c)
               (let* ([f (closure-fun c)]
                      [new-env (cons (cons (fun-formal f) v) (closure-env c))])
                 (eval-under-env (fun-body f)
                                 (if (fun-nameopt f)
                                     (cons (cons (fun-nameopt f) c) new-env)
                                     new-env)))
               (error "MUPL call with non-fun")))]
        [(fun? e) (closure env e)]
        [(apair? e)
...
(define (mlet* lstlst e2)
  (define (aux lst)
    (if (null? lst)
        e2
        (let ([pr (car lst)])
          (mlet (car pr) (cdr pr) (aux  (cdr lst))))))
  (aux lstlst))
